  ...
  // Reservas (persistidas en localStorage para la demo)
  ...
  const STORAGE_RESERVAS_KEY = 'galiservas.reservas';

  function loadReservas(){
    try {
      const raw = localStorage.getItem(STORAGE_RESERVAS_KEY);
      if(raw){
        const d = JSON.parse(raw);
        if(Array.isArray(d)) return d;
      }
    } catch {}
    return RESERVAS_INICIALES.map(r => ({ ...r }));
  }

  function saveReservas(){
    try { localStorage.setItem(STORAGE_RESERVAS_KEY, JSON.stringify(reservations)); } catch {}
  }

  const reservations = loadReservas();
  ...
  // ---------- SESION SSO (Galisencia) ----------
  const SESSION_KEY = 'galisencia.session';
  const BOOKING_ROLES = ['alumno', 'preceptor', 'directivo'];
  ...
  const canBook = !!sessionUser && BOOKING_ROLES.includes(sessionUser.rol);
  ...
  function renderSessionChip(){
    ...
    if(sessionUser){
      const labels = { alumno: 'Alumno', preceptor: 'Preceptor', directivo: 'Directivo', admin: 'Administrador' };
      chip.textContent = `🔐 ${sessionUser.nombre} · ${labels[sessionUser.rol] || sessionUser.rol}`;
      chip.classList.add(canBook ? 'chip-ok' : 'chip-warn');
    } else {
      chip.textContent = '🔐 Sin sesión: iniciá sesión en Galisencia para reservar';
  ...
  function updateConfirmBar(){
    ...
    if(!canBook){
      summary.textContent = sessionUser
        ? 'Los administradores consultan; las reservas las realizan alumnos, preceptores y directivos.'
        : 'Iniciá sesión en Galisencia como alumno, preceptor o directivo para reservar.';
      btn.disabled = true;
      return;
    }
  ...
  function confirmBooking(){
    if(!canBook){
      showToast('Iniciá sesión en Galisencia como alumno, preceptor o directivo para reservar');
      return;
    }
    ...
    saveReservas();
    showToast(`Reserva confirmada: ${selectedRoomName} ✓`);
