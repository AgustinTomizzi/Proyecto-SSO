...
if ($method === "PUT") {
    ...
    $stmt = $pdo->prepare("UPDATE alumnos SET nombre = ?, email = ?, curso_id = ? WHERE id_alumno = ?");
    $stmt->execute([$nombre, $email, $curso_id, $id]);
    api_json(["ok" => true]);
}
